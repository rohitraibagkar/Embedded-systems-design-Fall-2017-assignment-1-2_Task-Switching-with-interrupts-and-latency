...
unsigned long timer[5]; /*  An array of five elements to keep reference start time ... */
...
void loop() /*  function to run continiously... */
{
  ...
  {
    Open_Open_Open(); /*  functios to blink all LEDs is being called... */
  }
  else if (!digitalRead(51) && !digitalRead(52) && digitalRead(53)) /*  else checking if switch 1 is closed... */
  {
    sequential_blinking(2000, 500); /*  calling function to blink LEDs in sequence with 1 flash/second... */
  }
  else if (!digitalRead(51) && digitalRead(52) && !digitalRead(53)) /*  else checking if switch 2 is closed... */
  {
    sequential_blinking(2000, 250); /*  calling function to blink LEDs in sequence with 2 flash/second... */
  }
  else if (!digitalRead(51) && digitalRead(52) && digitalRead(53)) /*  else checking if switch 1 & 2 is closed... */
  {
    sequential_blinking(2000, 125); /*  calling function to blink LEDs in sequence with 4 flash/second... */
  }
  else if (digitalRead(51) && !digitalRead(52) && !digitalRead(53)) /*  else checking if switch 3 is closed... */
  {
    blinking_collision(2000, 500); /*  calling function to blink LEDs in collision manner with 1 flash/second... */
  }
  else if (digitalRead(51) && !digitalRead(52) && digitalRead(53)) /*  else checking if switch 3 & 1 is closed... */
  {
    blinking_collision(2000, 250); /*  calling function to blink LEDs in collision manner with 2 flash/second... */
  }
  else if (digitalRead(51) && digitalRead(52) && !digitalRead(53)) /*  else checking if switch 3 & 2 is closed... */
  {
    blinking_collision(2000, 125); /*  calling function to blink LEDs in collision manner with 4 flash/second... */
  }
  else if (digitalRead(51) && digitalRead(52) && digitalRead(53)) /*  else checking if switch 3 & 2 & 1 is closed... */
  {
    closed_closed_closed(); /*  Blinking all LEDs at different flash rates... */
  }
  /*sprintf(message, "SW 1 : %d, Current Value is : %d, Voltage is : %d", digitalRead(53), analogRead(0), analogRead(0));
  Serial.println(message);
  delay(1000);*/
}

void Open_Open_Open() /*  function to blink all LEDs at same time && same rate... */
{
  my_LED_blinker(led_1, 500, 0); /*  calling function to blink LED 1... */
  my_LED_blinker(led_2, 500, 1); /*  calling function to blink LED 2... */
  my_LED_blinker(led_3, 500, 2); /*  calling function to blink LED 3... */
  my_LED_blinker(led_4, 500, 3); /*  calling function to blink LED 4... */
  my_LED_blinker(led_5, 500, 4); /*  calling function to blink LED 5... */
}

void closed_closed_closed() /*  function to blink all the LEDs at same time && different rate... */
{
  my_LED_blinker(led_1, 500, 0); /*  calling function to blink LED 1 @ 1 flash per second... */
  my_LED_blinker(led_2, 250, 1); /*  calling function to blink LED 2 @ 2 flashes per second... */
  my_LED_blinker(led_3, 166, 2); /*  calling function to blink LED 3 @ 3 flashes per second... */
  my_LED_blinker(led_4, 125, 3); /*  calling function to blink LED 4 @ 4 flashes per second... */
  my_LED_blinker(led_5, 100, 4); /*  calling function to blink LED 5 @ 5 flashes per second... */
}
